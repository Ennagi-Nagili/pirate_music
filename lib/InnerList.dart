...
void main() {
  runApp(const InnerList(id: 0, name: ""));
}

class InnerList extends StatefulWidget {
  const InnerList({super.key, required this.id, required this.name});

  final int id;
  final String name;

  @override
  State<InnerList> createState() => _InnerListState(id: id, name: name);
}
...
class _InnerListState extends State<InnerList> {
  ...
  _InnerListState({required this.id, required this.name});

  Future<List<Store>?> getPlaylistData() {
    var path = "pirate_db2";
    var service = FileService(path: path);

    return service.getAllByListId(id);
  }
                          ...
                          child: Column(
                            ...
                            children: [
                              InkWell(
                                  onTap: () async {
                                    previous = index;
                                    onTapItem(snapshot.data[index].path);
                                    if (index != snapshot.data.length - 1) {
                                      next = snapshot.data[index + 1].path;
                                    } else {
                                      last = true;
                                    }
                                  },
                ...
                bottomNavigationBar: Visibility(
                    ...
                    child: Card(
                        ...
                        child:
                          ...
                          Row(children: [
                            IconButton(onPressed: playMusic, icon: playBack),
                            IconButton(
                                onPressed: skip,
                                icon: const Icon(
                                  Icons.skip_next,
                            ...
                            IconButton(
                                onPressed: onSoundClick, icon: soundBack),
                            Slider(
                                value: soundValue,
